...
class Batch extends CKFinder_Connector_CommandHandler_FileUpload {
    ...
    public function uploadFiles(&$command) {
        ...
        if ($command != "FileUpload") {
            return true;
        }  // if given command wasn't to upload files, don't use this plug in.

        if (empty($_FILES) || !isset($_FILES['upload']) || !is_array($_FILES['upload']['tmp_name'])) {
            return true;
        } // if only one file was uploaded through normal upload form, use regulard method for handling.
